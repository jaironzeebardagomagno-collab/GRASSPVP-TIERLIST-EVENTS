...
data.settings.auras={...DEFAULT_AURAS,...(data.settings.auras||{})};data.settings.tierAuras={...TIER_AURAS,...(data.settings.tierAuras||{})};
...
function iconFor(k){return iconSvg(k)}
function auraStyle(color){return `--aura:${esc(color||"#9b7cff")}`}
function tierBadge(kit,t,small=false){if(t==="No Tier")return `<span class="tier-empty">—</span>`;const color=data.settings.tierAuras[t]||TIER_AURAS[t]||"#9b7cff";return `<span class="tier-badge ${small?"tier-badge-small":""}" style="--tier-aura:${esc(color)}" title="${esc(kit)} • ${esc(t)}"><span class="tier-badge-glow"></span><b>${esc(t)}</b></span>`}
...
function renderPlayers(){
 ...
 $("playersGrid").innerHTML=players.length?players.map((p,i)=>{
  ...
  const tiers=KITS.map(([k])=>{const t=p.tiers?.[k]||"No Tier";return t!=="No Tier"?`<span class="player-tier-chip" style="--tier-aura:${esc(data.settings.tierAuras[t]||TIER_AURAS[t])}">${esc(k)} <b>${esc(t)}</b></span>`:""}).join("");
...
function renderSettings(){
 ...
 const tierRows=TIERS.filter(t=>t!=="No Tier");$("tierSettings").innerHTML=tierRows.map(t=>`<label class="rank-color"><b>${esc(t)}</b><input type="color" data-tier-aura="${esc(t)}" value="${esc(data.settings.tierAuras[t]||TIER_AURAS[t])}"></label>`).join("");
...
function saveSettings(){
 ...
 document.querySelectorAll("[data-tier-aura]").forEach(input=>data.settings.tierAuras[input.dataset.tierAura]=input.value);
